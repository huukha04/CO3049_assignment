...
document.addEventListener("DOMContentLoaded", function() {
    ...
    confirmBtn.addEventListener("click", function() {
        if (step === 2) {
            fetch('http://localhost/PHP_MVC/public/auth/getRole')
...
async function loadSeat() {
    try {
        const response = await fetch(`http://localhost/PHP_MVC/public/main/getSeat?showtime_id=${showtimeId}`);
            ...
            Object.entries(rowMap).forEach(([rowIndex, seats]) => {
                ...
                seats.forEach(seat => {
                    ...
                    button.onclick = async function() {
                        ...
                        if (isPending) {
                            const response = await fetch(`http://localhost/PHP_MVC/public/main/deleteOrderDetail`, {
                                method: "POST"
                                , body: dataForm
                            });
                            const data = await response.json();
                            ...
                            const response = await fetch(`http://localhost/PHP_MVC/public/main/insertOrderDetail`, {
                                method: "POST"
                                , body: dataForm
                            });
                            const data = await response.json();
...
async function loadBookingInfo() {
    try {
        const response = await fetch(`http://localhost/PHP_MVC/public/main/getBookingInfo?showtime_id=${showtimeId}&order_code=${orderCode}`);
        const data = await response.json();
        if (data.status) {
...
async function confirmPayment() {
    try {
        ...
        dataForm.append("order_code", orderCode);
        const response = await fetch(`http://localhost/PHP_MVC/public/main/confirmPayment`, {
            method: "POST"
            , body: dataForm
        });
        ...
        if (result.status) {
            ...
            window.location.href = `http://localhost/PHP_MVC/public/main/payment?data=${encodedData}`;
        } else {
...
async function loadProduct() {
    try {
        const response = await fetch(`http://localhost/PHP_MVC/public/main/getProductByShowtime?id=${showtimeId}`);
        const data = await response.json();
